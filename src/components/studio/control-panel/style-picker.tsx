...
interface StyleDef {
  ...
  prompt: string; promptZh: string;          // Gemini/GPT (full description)
  promptFlux: string; promptFluxZh: string;  // FLUX (scene-only, ~20-30 words)
}
...
const STYLES: StyleDef[] = [
  ...
  { key: "white", zh: "纯白底", en: "White BG", icon: Square,
    ...
    promptFlux: "Pure white #FFFFFF background, product centered with a soft natural ground shadow, soft even studio lighting 5500K, clean commercial e-commerce packshot. Keep the product identical to the reference image, only place it on a pure white background, 8K",
    promptFluxZh: "纯白#FFFFFF背景，产品居中带自然着地淡影，柔和均匀摄影棚灯光5500K，干净商业电商白底图。保持产品与参考图完全一致，仅换成纯白背景，8K" },
  ...
  { key: "scene", zh: "场景图", en: "Lifestyle", icon: Image,
    ...
    promptFlux: "Bright modern living room with natural window light from the left, product placed on a wooden table, warm neutral tones, shallow depth of field with softly blurred background, editorial home magazine quality. Keep the product identical to the reference image, only change the background to this bright modern room, 8K",
    promptFluxZh: "明亮现代客厅，左侧自然窗光，产品置于木桌上，暖中性色调，浅景深背景柔化，家居杂志编辑品质。保持产品与参考图完全一致，仅将背景换成这个明亮现代房间，8K" },
  ...
  { key: "in_use", zh: "模特图", en: "Model", icon: User,
    ...
    promptFlux: "A real person naturally holding and using the product in their hand, candid lifestyle moment, soft window daylight, relaxed modern home setting, the person's face softly out of focus, shallow depth of field on the product, photorealistic editorial lifestyle photography. Add a real person interacting with the product while keeping the product identical to the reference image, 8K",
    promptFluxZh: "一位真人自然地手持并使用产品，抓拍生活瞬间，柔和窗光，松弛现代家居环境，人物面部柔焦，焦点在产品的浅景深，照片级编辑生活方式摄影。添加一位与产品互动的真人，同时保持产品与参考图完全一致，8K" },
  ...
  { key: "angles", zh: "多角度", en: "Multi-Angle", icon: Grid3X3,
    ...
    promptFlux: "Multi-angle product photography on pure white background, showing front, side and top-down views of the same product in a clean grid layout, consistent studio lighting. Keep the product identical to the reference image, show it from multiple angles on white background, 8K",
    promptFluxZh: "多角度产品摄影，纯白背景，同一产品的正面、侧面、俯视图干净网格排布，统一摄影棚灯光。保持产品与参考图完全一致，在白底上展示多角度，8K" },
  ...
  { key: "marble", zh: "奢华风", en: "Luxury", icon: Gem,
    ...
    promptFlux: "Product on a white Carrara marble surface with natural grey veining, soft directional light from upper left, generous negative space, desaturated warm neutral tones, luxury editorial aesthetic. Keep the product identical to the reference image, only place it on a marble surface with luxury lighting, 8K",
    promptFluxZh: "产品置于白色卡拉拉大理石台面带天然灰纹，左上方柔定向光，大量留白，低饱和暖中性色调，奢华编辑美学。保持产品与参考图完全一致，仅换成大理石台面配奢华灯光，8K" },
  ...
  { key: "natural", zh: "户外自然", en: "Outdoor", icon: Sun,
    ...
    promptFlux: "Outdoor golden hour setting, product on mossy stone or weathered wood, warm sunset sunlight, soft bokeh greenery background, hazy atmosphere, natural organic aesthetic. Keep the product identical to the reference image, only place it in this golden hour outdoor scene, 8K",
    promptFluxZh: "黄金时段户外场景，产品置于苔藓石或风化木上，温暖落日阳光，柔和散景绿植背景，薄雾氛围，自然有机美学。保持产品与参考图完全一致，仅换成黄金时段户外场景，8K" },
  ...
  { key: "cosy", zh: "温馨家", en: "Cosy Home", icon: Home,
    ...
    promptFlux: "Warm cosy interior, product on a wooden surface, soft window light through linen curtain, natural linen and wood textures, candlelight, Scandinavian-Japandi hygge atmosphere. Keep the product identical to the reference image, only place it in this warm cosy interior, 8K",
    promptFluxZh: "温暖治愈室内，产品置于木质台面，透过亚麻帘的柔和窗光，天然亚麻和木纹质感，烛光，北欧侘寂治愈氛围。保持产品与参考图完全一致，仅换成温暖治愈室内场景，8K" },
  ...
  { key: "dark_moody", zh: "暗调高级", en: "Dark & Moody", icon: Coffee,
    ...
    promptFlux: "Product on a dark charcoal background, single dramatic spotlight from upper right, deep shadows with sculptural contrast, cinematic moody mood, luxury brand aesthetic. Keep the product identical to the reference image, only change the background to dark charcoal with dramatic spotlight, 8K",
    promptFluxZh: "产品置于深炭黑背景，右上单光源戏剧性聚光，深邃阴影雕塑感对比，电影级暗调氛围，奢侈品牌美学。保持产品与参考图完全一致，仅换成深炭黑背景配戏剧聚光，8K" },
  ...
  { key: "cyberpunk", zh: "赛博朋克", en: "Cyberpunk", icon: Coffee,
    ...
    promptFlux: "Cyberpunk scene, product on a wet reflective dark surface, cyan and magenta neon rim lights wrapping the product, volumetric fog, holographic grid background, Blade Runner aesthetic. Keep the product identical to the reference image, only place it in this cyberpunk neon scene, 8K",
    promptFluxZh: "赛博朋克场景，产品置于湿润反光深色表面，青品红双霓虹轮廓光包裹产品，体积雾，全息网格背景，银翼杀手美学。保持产品与参考图完全一致，仅换成赛博朋克霓虹场景，8K" },
  ...
  { key: "canvas", zh: "画布风", en: "Canvas Art", icon: Palette,
    ...
    promptFlux: "Watercolor painting style background, product on textured watercolor paper surface, flowing abstract watercolor pigment washes in blue ochre and sage, soft north window light, artist studio feel. Keep the product identical to the reference image, only change the background to a watercolor painting style, 8K",
    promptFluxZh: "水彩画风格背景，产品置于有纹理的水彩纸台面，蓝赭鼠尾草色流动抽象水彩颜料渲染，柔北窗光，艺术家工作室感。保持产品与参考图完全一致，仅将背景换成水彩画风格，8K" },
  ...
  { key: "kawaii", zh: "卡通风", en: "Kawaii", icon: Smile,
    ...
    promptFlux: "Kawaii cute cartoon illustration style background, candy pastel gradient pink to lavender to baby blue, floating tiny stars and hearts and clouds, soft diffused light, playful adorable atmosphere. Keep the product identical to the reference image, only change the background to this cute kawaii cartoon illustration style, 8K",
    promptFluxZh: "卡哇伊可爱卡通插画风格背景，糖果粉彩渐变粉到紫到婴儿蓝，漂浮小星星爱心和云朵，柔和漫射光，俏皮可爱氛围。保持产品与参考图完全一致，仅将背景换成这种可爱卡通插画风格，8K" },
  ...
  { key: "infographic", zh: "信息图", en: "Infographic", icon: Image,
    ...
    promptFlux: "E-commerce infographic image, product on white background on the left side, clean information panel on the right side with line icons and short feature text, modern Amazon A+ content layout. Keep the product identical to the reference image, place it on the left with an info panel on the right, 8K",
    promptFluxZh: "电商信息图，产品白底位于左侧，右侧干净信息面板带线条图标和简短卖点文字，现代Amazon A+内容布局。保持产品与参考图完全一致，置于左侧，右侧配信息面板，8K" },
];
